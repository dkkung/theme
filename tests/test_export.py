...
class TestSave:
    ...
    def test_save_metadata_on_by_default(self, simple_chart, tmp_path):
        # The human generation text lives in the SVG/PNG, not the JSON description.
        save(simple_chart, str(tmp_path / "out"), background=["light"])
        assert "Generated by" in (tmp_path / "out_light.svg").read_text()
    ...
    def test_save_metadata_can_be_disabled(self, simple_chart, tmp_path):
        ...
        assert "Generated by" not in (tmp_path / "out_light.svg").read_text()
    ...
    def test_json_description_is_raw_user_description_only(self, simple_chart, tmp_path):
        # Point 1: the JSON description carries the user's text only — the provenance and
        # report text are NOT duplicated there (they live structured under usermeta).
    ...
    def test_full_text_still_in_svg(self, simple_chart, tmp_path):
        # The user description + generation text both live in the SVG <desc>, in order.
        save(simple_chart, str(tmp_path / "out"), description="Figure 1", background=["light"])
        svg = (tmp_path / "out_light.svg").read_text()
        assert svg.index("Figure 1") < svg.index("Generated by")
