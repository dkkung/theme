...
```
Generated by username with analysis.py on YYYYMMDD at HH:MM:SS UTC using Python vX.Y.Z / altair vX.Y.Z / dysonsphere vX.Y.Z.
```
...
# description field: "Figure 1\nGenerated by username with analysis.py on YYYYMMDD ..."
```

**Structured metadata.** In addition to the human-readable string above, `ds.save()` embeds a machine-readable JSON block — `{"provenance": {...}, "statistics": [...]}` — in **all three** outputs, so each is self-contained:
...
- **PNG** — in an `iTXt dysonsphere` chunk.

The block has two keys:

- `provenance` — the generation facts as structured fields, in the same order as the text line (`user`, `script`, `timestamp` (ISO-8601), `python`, `altair`, `dysonsphere`), so you don't have to parse the text string.
- `statistics` — the structured records from any [`add_statistics()`](#adding-p-value-annotations) calls (per-group descriptives, the omnibus result, and every comparison with exact p-values and effect sizes). Read it back with `json.load(open("myplot_vegalite.json"))["usermeta"]["dysonsphere"]["statistics"]` — no text parsing, and trivial to turn into CSV/TSV.

The Vega-Lite JSON `description` field carries **only your `description=`** (not the generation/report text), since that would duplicate the structured block and clutter the chart's aria-label. Pass `saveMetadata=False` to suppress both the text and the structured metadata.
