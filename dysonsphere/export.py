...
def save(
    ...
    saveMetadata:
        If ``True`` (default), records generation + statistics metadata in two forms.

        **Human-readable text** — a generation info string (format: ``"Generated by
        <user> with <script> on <YYYYMMDD> at <HH:MM:SS> UTC using Python <ver> /
        altair <ver> / dysonsphere <ver>."``) plus any ``add_statistics`` report,
        prepended by your ``description``. This goes into the **SVG** ``<desc>`` element
        and the **PNG** ``iTXt Description`` chunk. In Jupyter, ``<script>`` is
        ``"<jupyter-notebook>"``; username falls back to ``"unknown_user"``.

        **Structured JSON** — a ``{"provenance": {...}, "statistics": [...]}`` block:

        - ``provenance`` — the generation facts as fields (``user``, ``script``,
          ``timestamp`` (ISO-8601), ``python``, ``altair``, ``dysonsphere``).
        - ``statistics`` — the structured records queued by ``add_statistics`` (groups,
          omnibus result, comparisons with exact p-values and effect sizes), so the
          stats can be read back without parsing the text report.

        This lands in the **Vega-Lite JSON** under ``usermeta.dysonsphere`` (merged into
        any ``usermeta`` already on the chart), in the **SVG** ``<metadata id="dysonsphere">``
        element (CDATA), and the **PNG** ``iTXt dysonsphere`` chunk — so every format is
        self-contained and machine-readable.

        Note: the Vega-Lite JSON ``description`` field carries **only your** ``description``
        (not the generation/report text) since that would duplicate the structured
        ``usermeta`` and pollute the chart's aria-label. ``saveMetadata=False`` suppresses
        both the text and the structured metadata.
    background:
    ...
    if saveMetadata:
        ...
        # Field order mirrors the JSON provenance object below:
        # user -> script -> timestamp -> python -> altair -> dysonsphere.
        _meta = (
            f"Generated by {_user} with {_script} on {_now.strftime('%Y%m%d at %H:%M:%S UTC')}"
            f" using Python {_py_ver} / altair {alt.__version__} / dysonsphere {_ds_ver}."
        )
    ...
    # on: the rendered text goes into the SVG/PNG human channels, and the structured
    # {provenance, statistics} block rides as JSON in the Vega-Lite usermeta, the SVG
    # <metadata> element, and the PNG dysonsphere iTXt chunk.  See statistics._REPORTS.
    ...
    #
    # The chart's `description` property (which only feeds the Vega-Lite JSON spec) is
    # set to the *user's* raw description only — the provenance and report text are NOT
    # duplicated there, because the JSON already carries them structured under
    # `usermeta.dysonsphere`.  The full human text (`_effective_desc`) goes only into
    # the SVG <desc> / PNG iTXt, which have no structured channel.  (Vega-Lite's SVG
    # renderer does not emit <desc> from this property anyway; save() injects it below.)
    ...
    try:
        ...
        for mode, suffix in [_background_map[b] for b in background]:
            ...
            with open(svg_path, encoding="utf-8") as f:
                svg_content = f.read()
            # Inject the human text (<desc>) and the structured JSON (<metadata>) right
            # after the opening <svg> tag.  A lambda replacement is used so backslashes or
            # braces inside the JSON are inserted literally (never treated as regex refs).
            _inserts = ""
            ...
            if _effective_desc is not None:
                _inserts += f"<desc>{html.escape(_effective_desc)}</desc>"
            if _inserts:
